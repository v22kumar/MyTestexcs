...
program --- DFCC, CCDL and SPIL verification, firmware on ARM9/LPC3250, DPRAM,
BSP, and a MIL-STD-1553B Python automation suite, all under DO-178B Level A.
Currently at TASL UAV R\&D, I work on UAV flight-control systems --- DFCC and
signal-conditioning cards, hardware--software integration, board bring-up,
RS422 Embedded C interface development, multi-protocol validation, and root-cause
analysis across the UAV LRU set.'' Note the structure: \emph{engineer first},
then \emph{BEL (fighter) domain}, then \emph{TASL (UAV) domain} --- never mix the
two. Keep CCDL/SPIL in the BEL half and board bring-up in the TASL half. Practise
to 90 seconds.
