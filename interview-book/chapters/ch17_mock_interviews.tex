...
\textbf{The repositioned pitch (memorise the shape, lead with engineering):}
``I'm an Embedded Systems Engineer with experience across fighter-aircraft and
UAV avionics. At BEL I worked on flight-critical avionics for the LCA Tejas
program (MK1/MK1A/MK2), on the DFCC AETS project --- verifying DFCC cards,
signal-conditioning cards and single-board computers, the CCDL and SPIL data
links over RS422, firmware on ARM9/LPC3250 with DPRAM and BSP, and a
MIL-STD-1553B Python automation suite, all under DO-178B Level A. Currently at
TASL UAV R\&D, I work on UAV autopilot and avionics --- developing and
integrating Embedded C firmware on STM32 and LPC1778, bringing up and debugging
autopilot boards, integrating the UAV units (TLM~50, CBLM, ERLM) and LRUs, and
doing multi-protocol bring-up and root-cause analysis.'' Note the structure:
\emph{engineer first}, then \emph{BEL (fighter / DFCC AETS) domain}, then
\emph{TASL (UAV / STM32 autopilot) domain} --- never mix the two. Keep
CCDL/SPIL/DFCC in the BEL half and STM32/autopilot/board-bring-up in the TASL
half. Practise to 90 seconds.
